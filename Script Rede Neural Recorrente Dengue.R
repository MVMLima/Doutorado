...
x <- x[-(1:3),]
head(x)
x = log(x) #Transformação necessária para usar RNR
...
#Criando LOOP para análise(NÂO USAR)
...
pred_actual = ts(matrix(pred_actual), end=c(2017,12), frequency=12)

result_all = cbind(result_all, round(pred_actual,2))
colnames(result_all) = c("actual", "Model 1","Model 2","One step")
